...
- `proc_isocity_osmimport`: import **OpenStreetMap (OSM XML)** features into a ProcIsoCity world (roads + optional water/landuse/parks/buildings).
  This rasterizes tagged OSM ways onto the tile grid, writes a save, and can optionally run the deterministic
  AutoBuild bot to grow zones around the imported network.

  Supported tag families (high-level):
  - Roads: `highway=*`
  - Water: `natural=water` / `water=*` polygons and `waterway=river|stream|canal|drain` lines
  - Landuse zones: `landuse=residential|commercial|industrial`
  - Parks: `leisure=park|garden` (+ some common green landuse tags)
  - Buildings: `building=*` polygons (with optional `building:levels` for density)

  ```bash
  # Roads only (original behavior)
  ./build/proc_isocity_osmimport --osm extract.osm --save osm_roads.bin --meters-per-tile 20

  # Full import: roads + water + landuse + parks + buildings (plus 120 days of deterministic growth)
  ./build/proc_isocity_osmimport --osm extract.osm --save osm_city.bin --size 512x512 \
    --full --overwrite 1 --autobuild-days 120
  ```
...
- **L**: cycle heatmap overlay (land value / park amenity / water amenity / pollution / traffic spill / flood depth / ponding depth)
