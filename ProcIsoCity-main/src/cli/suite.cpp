...
void PrintHelp()
{
  std::cout
      ...
      << "  --golden-ssim-window <N>      SSIM window size (odd >=3). Default: 11\n"
      << "  --golden-ssim-min <f>         Consider a golden MATCH if SSIM >= f (0..1).\n"
...
struct GoldenConfig {
  ...
  // SSIM (luma) parameters. If ssimMin >= 0, we treat a golden as matching when
  // either pixelsDifferent==0 OR ssim>=ssimMin. This is useful for allowing
  // small rendering drift while still catching meaningful differences.
  int ssimWindow = 11;   // odd >= 3
  double ssimMin = -1.0; // disabled when < 0 (otherwise expected 0..1)
...
std::string FormatPpmDiffSummary(const isocity::PpmDiffStats& st)
{
  ...
  oss << ", ssim=" << std::fixed << std::setprecision(6) << st.ssim;
  oss << ")";
  return oss.str();
}
      ...
      if (golden.update) {
        ...
        if (goldenExists) {
          ...
          if (isocity::ReadImageAuto(goldenPath.string(), expected, readErr)) {
            ...
            if (isocity::ComparePpm(expected, actual, st, golden.threshold, nullptr, golden.ssimWindow)) {
              cr.golden.stats = st;
              cr.golden.hasStats = true;
              const bool matched = (st.pixelsDifferent == 0) || (golden.ssimMin >= 0.0 && st.ssim >= golden.ssimMin);
              needsWrite = !matched;
            } else {
              needsWrite = true;
            }
        ...
        if (!goldenExists) {
          ...
          if (!isocity::ReadImageAuto(goldenPath.string(), expected, readErr)) {
            ...
            if (!isocity::ComparePpm(expected, actual, st, golden.threshold, diffPtr, golden.ssimWindow)) {
              cr.golden.ok = false;
              cr.golden.matched = false;
...
int main(int argc, char** argv)
{
  ...
  for (int i = 1; i < argc; ++i) {
    ...
    } else if (arg == "--golden-ssim-window") {
      if (!requireValue(i, val)) {
        std::cerr << "--golden-ssim-window requires an integer\n";
        return 2;
      }
      golden.enabled = true;
      if (!ParseI32(val, &golden.ssimWindow) || golden.ssimWindow < 3 || golden.ssimWindow > 999) {
        std::cerr << "invalid --golden-ssim-window (expected odd integer >=3): " << val << "\n";
        return 2;
      }
      if ((golden.ssimWindow % 2) == 0) {
        // enforce odd window size for symmetric radius
        ++golden.ssimWindow;
      }
    } else if (arg == "--golden-ssim-min") {
      if (!requireValue(i, val)) {
        std::cerr << "--golden-ssim-min requires a float\n";
        return 2;
      }
      golden.enabled = true;
      if (!ParseF64(val, &golden.ssimMin) || golden.ssimMin < 0.0 || golden.ssimMin > 1.0) {
        std::cerr << "invalid --golden-ssim-min (expected 0..1): " << val << "\n";
        return 2;
      }
