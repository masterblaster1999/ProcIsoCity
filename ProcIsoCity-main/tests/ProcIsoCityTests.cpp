...
void TestExportPpmLayers()
{
  ...
  EXPECT_TRUE(ParseExportLayer("flood_depth", layer));
  EXPECT_EQ(layer, ExportLayer::FloodDepth);

  EXPECT_TRUE(ParseExportLayer("ponding_depth", layer));
  EXPECT_EQ(layer, ExportLayer::PondingDepth);
...
void TestPpmReadWriteAndCompare()
{
  ...
  // SSIM sanity: identical images should have SSIM == 1.
  PpmDiffStats stSame{};
  EXPECT_TRUE(ComparePpm(img, img, stSame, /*threshold=*/0, nullptr));
  EXPECT_NEAR(stSame.ssim, 1.0, 1e-12);
  ...
  // SSIM should drop below 1 when we introduce a difference.
  EXPECT_TRUE(st.ssim < 1.0);
...
void TestOsmImportBasic()
{
  ...
  // A tiny OSM snippet: one primary road polyline across the bounds.
  // The importer should rasterize at least one road tile and preserve the level mapping.
  const char* kOsm =
  ...
  const fs::path osmPath = fs::path("proc_isocity_test_osmimport.osm");
  {
    std::ofstream o(osmPath, std::ios::binary);
    o.write(kOsm, static_cast<std::streamsize>(std::strlen(kOsm)));
  }
  ...
  cfg.thickenByClass = false;

  World w(cfg.width, cfg.height, 1234u);

  OsmImportStats stats;
  std::string err;
  EXPECT_TRUE(ImportOsmXmlRoads(osmPath.string(), w, cfg, &stats, err));
  EXPECT_TRUE(err.empty());
  EXPECT_TRUE(stats.roadTilesPainted > 0u);

  // The road should include at least one level-3 tile (primary => Highway class).
  int maxLevel = 0;
  ...
  for (int y = 0; y < w.height(); ++y) {
    for (int x = 0; x < w.width(); ++x) {
      ...
      if (t.overlay == Overlay::Road) {
        counted++;
        maxLevel = std::max(maxLevel, static_cast<int>(t.level));
      }
    }
  }
  EXPECT_EQ(counted, stats.roadTilesPainted);
...
void TestOsmImportLanduseWaterAndBuildings()
{
  using namespace isocity;
  namespace fs = std::filesystem;

  // Synthetic OSM extract that includes:
  //   - landuse polygon (commercial)
  //   - building polygon (apartments, levels)
  //   - water polygon (lake)
  //   - park polygon
  //   - highway line
  //   - waterway line
  const char* kOsm = R"(<?xml version="1.0" encoding="UTF-8"?>
<osm version="0.6" generator="ProcIsoCityTests">
  <bounds minlat="0.0" minlon="0.0" maxlat="1.0" maxlon="1.0" />
  <!-- Commercial landuse polygon -->
  <node id="1" lat="0.2" lon="0.2" />
  <node id="2" lat="0.2" lon="0.6" />
  <node id="3" lat="0.6" lon="0.6" />
  <node id="4" lat="0.6" lon="0.2" />
  <!-- Building polygon inside -->
  <node id="10" lat="0.3" lon="0.3" />
  <node id="11" lat="0.3" lon="0.4" />
  <node id="12" lat="0.4" lon="0.4" />
  <node id="13" lat="0.4" lon="0.3" />
  <!-- Water polygon -->
  <node id="20" lat="0.7" lon="0.7" />
  <node id="21" lat="0.7" lon="0.9" />
  <node id="22" lat="0.9" lon="0.9" />
  <node id="23" lat="0.9" lon="0.7" />
  <!-- Park polygon -->
  <node id="30" lat="0.65" lon="0.15" />
  <node id="31" lat="0.65" lon="0.25" />
  <node id="32" lat="0.85" lon="0.25" />
  <node id="33" lat="0.85" lon="0.15" />
  <!-- Highway line -->
  <node id="40" lat="0.1" lon="0.1" />
  <node id="41" lat="0.95" lon="0.1" />
  <!-- Waterway line -->
  <node id="50" lat="0.5" lon="0.1" />
  <node id="51" lat="0.5" lon="0.95" />
  ...
  const fs::path osmPath = fs::temp_directory_path() / "procisocity_test_osm_features.osm";
  {
    std::ofstream f(osmPath);
    EXPECT_TRUE(f.good());
    f << kOsm;
  }
  ...
  // Turn on the new feature import layers.
  ...
  // Let buildings override landuse where they overlap.
  cfg.overwriteNonRoadOverlays = true;

  // Keep tests deterministic & crisp.
  cfg.thickenByClass = false;
  cfg.fixedRadius = 0;
  cfg.waterwayRadius = 1;

  World w(96, 96, 1234);
  OsmImportStats stats;
  std::string err;
  EXPECT_TRUE(ImportOsmXmlRoads(osmPath.string(), w, cfg, &stats, err)) << err;

  EXPECT_TRUE(stats.roadTilesPainted > 0);
  EXPECT_TRUE(stats.waterTilesPainted > 0);
  EXPECT_TRUE(stats.zoneTilesPainted > 0);
  EXPECT_TRUE(stats.parkTilesPainted > 0);
  ...
  for (int y = 0; y < w.height(); ++y) {
    for (int x = 0; x < w.width(); ++x) {
      const Tile& t = w.at(x, y);
      if (t.overlay == Overlay::Residential) foundRes = true;
      if (t.overlay == Overlay::Commercial) foundCom = true;
      if (t.terrain == Terrain::Water) {
        // Water should not contain zoning/parks (roads are allowed as bridges).
        EXPECT_TRUE(t.overlay == Overlay::None || t.overlay == Overlay::Road);
      }
    }
  }
  EXPECT_TRUE(foundRes);
  EXPECT_TRUE(foundCom);
...
int main()
{
  ...
  TestOsmImportBasic();
  TestOsmImportLanduseWaterAndBuildings();
