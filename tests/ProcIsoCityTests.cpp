...
void TestProcGenTerrainPresetAtollCreatesLagoonAndPass()
{
  using namespace isocity;
  ...
  ProcGenTerrainPreset p2{};
  EXPECT_TRUE(ParseProcGenTerrainPreset("reef", p2));
  EXPECT_EQ(p2, ProcGenTerrainPreset::Atoll);
  ...
  const std::uint64_t seed = 0xA70011E1u;
  const World w = GenerateWorld(64, 64, seed, pc);
  ...
  const float cx = (static_cast<float>(w.width()) - 1.0f) * 0.5f;
  const float cy = (static_cast<float>(w.height()) - 1.0f) * 0.5f;
  ...
  for (int y = 0; y < w.height(); ++y) {
    for (int x = 0; x < w.width(); ++x) {
      ...
      const Tile& t = w.at(x, y);
      if (t.terrain != Terrain::Water) ++landTotal;

      if (r < 0.45f) {
        ++center;
        if (t.terrain == Terrain::Water) ++centerWater;
      }

      if (r > 0.68f && r < 0.90f) {
        ++ring;
        if (t.terrain != Terrain::Water) ++ringLand;
      }
    }
  }

  const float borderFrac = (border > 0) ? (static_cast<float>(borderWater) / static_cast<float>(border)) : 0.0f;
  ...
  const float landFrac = (total > 0) ? (static_cast<float>(landTotal) / static_cast<float>(total)) : 0.0f;

  // Edges should be mostly ocean; the center should be mostly lagoon water.
  EXPECT_TRUE(borderFrac > 0.55f);
  EXPECT_TRUE(centerFrac > 0.55f);

  // There should be a distinct land ring.
  EXPECT_TRUE(ringFrac > 0.25f);
  EXPECT_TRUE(landFrac > 0.10f);

  // The lagoon should connect to the ocean via at least one pass.
  int sx = -1;
  int sy = -1;
  const int cxI = w.width() / 2;
  const int cyI = w.height() / 2;
  for (int dy = -6; dy <= 6 && sx < 0; ++dy) {
    for (int dx = -6; dx <= 6 && sx < 0; ++dx) {
      const int x = cxI + dx;
      const int y = cyI + dy;
      if (x < 0 || y < 0 || x >= w.width() || y >= w.height()) continue;
      if (w.at(x, y).terrain == Terrain::Water) {
        sx = x;
        sy = y;
      }
    }
  ...
  bool reached = false;
  if (sx >= 0) {
    std::vector<std::uint8_t> seen(static_cast<std::size_t>(total), 0u);
    std::queue<std::pair<int, int>> q;
    q.push({sx, sy});
    seen[static_cast<std::size_t>(sy * w.width() + sx)] = 1u;

    auto push = [&](int x, int y) {
      if (x < 0 || y < 0 || x >= w.width() || y >= w.height()) return;
      if (w.at(x, y).terrain != Terrain::Water) return;
      const std::size_t ii = static_cast<std::size_t>(y * w.width() + x);
      if (seen[ii]) return;
      seen[ii] = 1u;
      q.push({x, y});
    };
...
void TestProcGenTerrainPresetPeninsulaCreatesMainlandEdgeAndTaper()
{
  using namespace isocity;

  ProcGenTerrainPreset p{};
  EXPECT_TRUE(ParseProcGenTerrainPreset("peninsula", p));
  EXPECT_EQ(p, ProcGenTerrainPreset::Peninsula);
  EXPECT_EQ(std::string(ToString(p)), std::string("peninsula"));

  ProcGenTerrainPreset p2{};
  EXPECT_TRUE(ParseProcGenTerrainPreset("cape", p2));
  EXPECT_EQ(p2, ProcGenTerrainPreset::Peninsula);
  ...
  pc.erosion.enabled = false;
  pc.erosion.riversEnabled = false;
  pc.terrainPreset = ProcGenTerrainPreset::Peninsula;
  pc.terrainPresetStrength = 1.25f;

  // Reduce incidental lakes so the test focuses on the peninsula macro-shape.
  pc.waterLevel = 0.20f;
  pc.sandLevel = 0.28f;

  const std::uint64_t seed = 0x50454E15u;
  const World w = GenerateWorld(96, 72, seed, pc);

  const int W = w.width();
  const int H = w.height();
  const int N = W * H;

  auto isLand = [&](int x, int y) -> bool { return w.at(x, y).terrain != Terrain::Water; };

  auto edgeLandFrac = [&](int edge) -> float {
    int land = 0;
    int tot = 0;
    if (edge == 0) { // left
      for (int y = 0; y < H; ++y) {
      ...
      for (int x = 0; x < W; ++x) {
        ++tot;
        if (isLand(x, H - 1)) ++land;
      }
    }
    return (tot > 0) ? (static_cast<float>(land) / static_cast<float>(tot)) : 0.0f;
  };

  float fr[4] = {edgeLandFrac(0), edgeLandFrac(1), edgeLandFrac(2), edgeLandFrac(3)};
  int mainEdge = 0;
  float mainFrac = fr[0];
  float minFrac = fr[0];
  ...
  for (int e = 1; e < 4; ++e) {
    ...
    minFrac = std::min(minFrac, fr[e]);
  }

  // The peninsula preset should create one "mainland" edge with noticeably more land.
  EXPECT_TRUE(mainFrac > 0.35f);
  EXPECT_TRUE(mainFrac > minFrac + 0.18f);

  // Non-mainland edges should skew toward water.
  for (int e = 0; e < 4; ++e) {
    if (e == mainEdge) continue;
    EXPECT_TRUE(fr[e] < 0.35f);
  }

  // BFS from mainland edge land tiles; ensure the landmass reaches deep into the map.
  std::vector<std::uint8_t> vis(static_cast<std::size_t>(N), 0);
  std::vector<int> q;
  q.reserve(static_cast<std::size_t>(W + H));

  auto idx = [W](int x, int y) { return x + y * W; };

  auto pushIfLand = [&](int x, int y) {
    if (!isLand(x, y)) return;
    const int id = idx(x, y);
    if (vis[static_cast<std::size_t>(id)]) return;
    vis[static_cast<std::size_t>(id)] = 1;
    q.push_back(id);
  };

  if (mainEdge == 0) {
    for (int y = 0; y < H; ++y) pushIfLand(0, y);
  } else if (mainEdge == 1) {
    for (int y = 0; y < H; ++y) pushIfLand(W - 1, y);
  } else if (mainEdge == 2) {
    for (int x = 0; x < W; ++x) pushIfLand(x, 0);
  } else {
    for (int x = 0; x < W; ++x) pushIfLand(x, H - 1);
  }

  int reach = 0;
  if (mainEdge == 1) reach = W - 1;
  if (mainEdge == 3) reach = H - 1;
  ...
  for (std::size_t qi = 0; qi < q.size(); ++qi) {
    ...
    if (mainEdge == 0) reach = std::max(reach, x);
    if (mainEdge == 1) reach = std::min(reach, x);
    if (mainEdge == 2) reach = std::max(reach, y);
    if (mainEdge == 3) reach = std::min(reach, y);
  ...
  if (mainEdge == 0) {
    EXPECT_TRUE(reach > static_cast<int>(std::round(static_cast<float>(W) * 0.55f)));
  } else if (mainEdge == 1) {
    EXPECT_TRUE(reach < static_cast<int>(std::round(static_cast<float>(W) * 0.45f)));
  } else if (mainEdge == 2) {
    EXPECT_TRUE(reach > static_cast<int>(std::round(static_cast<float>(H) * 0.55f)));
  } else {
    EXPECT_TRUE(reach < static_cast<int>(std::round(static_cast<float>(H) * 0.45f)));
  }

  // Water should be a significant feature for this preset.
  int waterCount = 0;
  for (int y = 0; y < H; ++y) {
    for (int x = 0; x < W; ++x) {
      if (w.at(x, y).terrain == Terrain::Water) ++waterCount;
    }
  }
  EXPECT_TRUE(waterCount > (N / 5));
}
...
static void TestStreetNamesAndAddressesDeterministic()
{
  using namespace isocity;

  World world = GenerateWorld(64, 64, 424242ULL);

  StreetNamingConfig scfg;
  scfg.mergeThroughIntersections = true;
  scfg.mergeThroughCorners = true;
  scfg.allowOrdinalNames = true;

  const StreetNamingResult a = BuildStreetNames(world, scfg);
  const StreetNamingResult b = BuildStreetNames(world, scfg);

  EXPECT_EQ(a.w, b.w);
  EXPECT_EQ(a.h, b.h);
  EXPECT_EQ(a.streets.size(), b.streets.size());
  EXPECT_EQ(a.roadTileToStreetId.size(), b.roadTileToStreetId.size());

  // Stable mapping.
  for (std::size_t i = 0; i < a.roadTileToStreetId.size(); ++i) {
    EXPECT_EQ(a.roadTileToStreetId[i], b.roadTileToStreetId[i]);
  }

  // Stable street list (names + metrics).
  for (std::size_t i = 0; i < a.streets.size(); ++i) {
    const StreetInfo& sa = a.streets[i];
    const StreetInfo& sb = b.streets[i];
    EXPECT_EQ(sa.id, sb.id);
    EXPECT_EQ(sa.roadLevel, sb.roadLevel);
    EXPECT_EQ(sa.edgeCount, sb.edgeCount);
    EXPECT_EQ(sa.tileCount, sb.tileCount);
    EXPECT_EQ(sa.axis, sb.axis);
    EXPECT_EQ(sa.axisMin, sb.axisMin);
    EXPECT_EQ(sa.name, sb.name);
  }

  const std::vector<ParcelAddress> aa = BuildParcelAddresses(world, a);
  const std::vector<ParcelAddress> ab = BuildParcelAddresses(world, b);

  EXPECT_TRUE(!aa.empty());
  EXPECT_EQ(aa.size(), ab.size());

  const std::size_t k = std::min<std::size_t>(aa.size(), 64u);
  for (std::size_t i = 0; i < k; ++i) {
    EXPECT_EQ(aa[i].parcelIndex, ab[i].parcelIndex);
    EXPECT_EQ(aa[i].streetId, ab[i].streetId);
    EXPECT_EQ(aa[i].houseNumber, ab[i].houseNumber);
    EXPECT_EQ(aa[i].streetName, ab[i].streetName);
    EXPECT_EQ(aa[i].full, ab[i].full);
    EXPECT_EQ(aa[i].roadTile.x, ab[i].roadTile.x);
    EXPECT_EQ(aa[i].roadTile.y, ab[i].roadTile.y);
  }
}

static void TestGfxFacilitiesDeterministic()
{
  using namespace isocity;

  GfxFacilitiesConfig cfg;
  cfg.tileW = 64;
  cfg.tileH = 32;
  cfg.spriteH = 0; // auto
  cfg.includeEmissive = true;

  const GfxPalette pal = GenerateGfxPalette(123456u, GfxTheme::Classic);

  const GfxFacilityKind kinds[] = {
      GfxFacilityKind::Education,
      GfxFacilityKind::Health,
      GfxFacilityKind::Police,
      GfxFacilityKind::Fire,
  };

  for (GfxFacilityKind k : kinds) {
    for (int lvl = 1; lvl <= 3; ++lvl) {
      // A few variants to keep runtime low, but still cover diversity.
      for (int v : {0, 1, 7}) {
        GfxFacilitySprite a;
        GfxFacilitySprite b;
        std::string err;

        EXPECT_TRUE(GenerateGfxFacilitySprite(k, lvl, v, 999u, cfg, pal, a, err));
        EXPECT_TRUE(GenerateGfxFacilitySprite(k, lvl, v, 999u, cfg, pal, b, err));

        EXPECT_EQ(a.color.width, cfg.tileW);
        EXPECT_EQ(a.color.width, b.color.width);
        EXPECT_EQ(a.color.height, b.color.height);
        EXPECT_TRUE(!a.color.rgba.empty());
        EXPECT_TRUE(a.color.rgba.size() == b.color.rgba.size());

        // Deterministic pixels.
        const std::uint32_t crca = Crc32(a.color.rgba.data(), a.color.rgba.size());
        const std::uint32_t crcb = Crc32(b.color.rgba.data(), b.color.rgba.size());
        EXPECT_EQ(crca, crcb);

        EXPECT_TRUE(a.pivotX >= 0 && a.pivotX < a.color.width);
        EXPECT_TRUE(a.pivotY >= 0 && a.pivotY < a.color.height);

        // Some non-transparent pixels must exist.
        int solid = 0;
        for (std::size_t i = 0; i + 3 < a.color.rgba.size(); i += 4) {
          if (a.color.rgba[i + 3] > 0) {
            ++solid;
            if (solid > 64) break;
          }
        }
        EXPECT_TRUE(solid > 64);

        // Emissive atlas should be present and deterministic too.
        EXPECT_EQ(a.emissive.width, a.color.width);
        EXPECT_EQ(a.emissive.height, a.color.height);
        EXPECT_TRUE(a.emissive.rgba.size() == a.color.rgba.size());
        const std::uint32_t erca = Crc32(a.emissive.rgba.data(), a.emissive.rgba.size());
        const std::uint32_t ercb = Crc32(b.emissive.rgba.data(), b.emissive.rgba.size());
        EXPECT_EQ(erca, ercb);
      }
    }
  }
...
static void TestCartographyPosterDeterministic()
{
  using namespace isocity;

  World world = GenerateWorld(64, 64, 777777ULL);
  ...
  streetCfg.mergeThroughIntersections = true;
  streetCfg.mergeThroughCorners = true;
  streetCfg.allowOrdinalNames = true;

  CartographyConfig cfg;
  cfg.poster = true;
  cfg.labelTitle = true;
  cfg.labelDistricts = true;
  cfg.labelStreets = true;
  cfg.drawDistrictBoundaries = true;
  cfg.maxStreetLabels = 24;
  cfg.maxDistrictLabels = kDistrictCount;
  cfg.streetTextScale = 2;
  cfg.districtTextScale = 3;
  cfg.titleTextScale = 4;

  const CartographyResult a = RenderLabeledIsoPoster(world, ExportLayer::Overlay, isoCfg, streetCfg, cfg);
  const CartographyResult b = RenderLabeledIsoPoster(world, ExportLayer::Overlay, isoCfg, streetCfg, cfg);

  EXPECT_EQ(a.title, b.title);
  EXPECT_EQ(a.districtNames.size(), b.districtNames.size());
  EXPECT_EQ(a.districtNames.size(), static_cast<std::size_t>(kDistrictCount));
  for (std::size_t i = 0; i < a.districtNames.size(); ++i) {
    EXPECT_EQ(a.districtNames[i], b.districtNames[i]);
  }

  EXPECT_EQ(a.image.width, b.image.width);
  EXPECT_EQ(a.image.height, b.image.height);
  EXPECT_EQ(a.image.rgba.size(), b.image.rgba.size());
  EXPECT_TRUE(!a.image.rgba.empty());

  const std::uint32_t crca = Crc32(a.image.rgba.data(), a.image.rgba.size());
  const std::uint32_t crcb = Crc32(b.image.rgba.data(), b.image.rgba.size());
  EXPECT_EQ(crca, crcb);

  EXPECT_EQ(a.labels.size(), b.labels.size());
  EXPECT_TRUE(!a.labels.empty());

  // Spot check first few labels for deterministic placement.
  const std::size_t k = std::min<std::size_t>(a.labels.size(), 32u);
  for (std::size_t i = 0; i < k; ++i) {
    EXPECT_EQ(static_cast<int>(a.labels[i].kind), static_cast<int>(b.labels[i].kind));
    EXPECT_EQ(a.labels[i].id, b.labels[i].id);
    EXPECT_EQ(a.labels[i].text, b.labels[i].text);
    EXPECT_EQ(a.labels[i].x, b.labels[i].x);
    EXPECT_EQ(a.labels[i].y, b.labels[i].y);
    EXPECT_EQ(a.labels[i].w, b.labels[i].w);
    EXPECT_EQ(a.labels[i].h, b.labels[i].h);
    EXPECT_EQ(a.labels[i].anchorX, b.labels[i].anchorX);
    EXPECT_EQ(a.labels[i].anchorY, b.labels[i].anchorY);
    EXPECT_EQ(a.labels[i].scale, b.labels[i].scale);
  }

  // No overlaps among non-title labels.
  for (std::size_t i = 0; i < a.labels.size(); ++i) {
    if (a.labels[i].kind == MapLabelKind::Title) continue;
    for (std::size_t j = i + 1; j < a.labels.size(); ++j) {
      if (a.labels[j].kind == MapLabelKind::Title) continue;
      const MapLabel& A = a.labels[i];
      const MapLabel& B = a.labels[j];
      const bool overlapX = !(A.x + A.w - 1 < B.x || B.x + B.w - 1 < A.x);
      const bool overlapY = !(A.y + A.h - 1 < B.y || B.y + B.h - 1 < A.y);
      EXPECT_TRUE(!(overlapX && overlapY));
    }
  }
}

static void TestWayfindingRouteDeterministic()
{
  using namespace isocity;

  World world = GenerateWorld(64, 64, 20250101ULL);
  ...
  const std::vector<ParcelAddress> addrs = BuildParcelAddresses(world, streets);
  EXPECT_TRUE(!addrs.empty());

  AddressIndexConfig icfg;
  icfg.allowFuzzy = true;
  icfg.maxSuggestions = 5;
  icfg.maxAutoEditDistance = 4;

  const AddressIndex index = BuildAddressIndex(addrs, icfg);

  // Pick a deterministic pair of addresses that are connected by the road network.
  RouteResult a;
  bool found = false;

  const std::size_t limit = std::min<std::size_t>(addrs.size(), 200u);
  for (std::size_t i = 0; i < limit && !found; ++i) {
    for (std::size_t j = i + 1; j < limit && !found; ++j) {
      RouteResult r = RouteBetweenEndpoints(world, streets, addrs[i], addrs[j]);
      if (r.ok && r.pathTiles.size() >= 2u) {
        a = r;
        found = true;
      }
    }
  ...
  // Deterministic rerun.
  const RouteResult b = RouteBetweenEndpoints(world, streets, a.from, a.to);
  EXPECT_TRUE(b.ok);

  EXPECT_EQ(a.pathCost, b.pathCost);
  EXPECT_EQ(a.pathTiles.size(), b.pathTiles.size());
  EXPECT_EQ(a.maneuvers.size(), b.maneuvers.size());

  for (std::size_t i = 0; i < a.pathTiles.size(); ++i) {
    EXPECT_EQ(a.pathTiles[i].x, b.pathTiles[i].x);
    EXPECT_EQ(a.pathTiles[i].y, b.pathTiles[i].y);
  }

  const std::size_t mk = std::min<std::size_t>(a.maneuvers.size(), 32u);
  for (std::size_t i = 0; i < mk; ++i) {
    EXPECT_EQ(a.maneuvers[i].type, b.maneuvers[i].type);
    EXPECT_EQ(a.maneuvers[i].modifier, b.maneuvers[i].modifier);
    EXPECT_EQ(a.maneuvers[i].bearingBefore, b.maneuvers[i].bearingBefore);
    EXPECT_EQ(a.maneuvers[i].bearingAfter, b.maneuvers[i].bearingAfter);
    EXPECT_EQ(a.maneuvers[i].steps, b.maneuvers[i].steps);
    EXPECT_EQ(a.maneuvers[i].streetId, b.maneuvers[i].streetId);
    EXPECT_EQ(a.maneuvers[i].streetName, b.maneuvers[i].streetName);
    EXPECT_EQ(a.maneuvers[i].pathStart, b.maneuvers[i].pathStart);
    EXPECT_EQ(a.maneuvers[i].pathEnd, b.maneuvers[i].pathEnd);
    EXPECT_EQ(a.maneuvers[i].instruction, b.maneuvers[i].instruction);
  }
}

static void TestProceduralTourDeterministic()
{
  using namespace isocity;

  World world = GenerateWorld(64, 64, 20250202ULL);

  TourConfig cfg;
  cfg.maxStops = 6;
  cfg.minSeparationTiles = 8;
  cfg.centralityMaxSources = 24; // keep the test fast & deterministic
  cfg.seedSalt = 0;

  const TourPlan a = BuildProceduralTour(world, "", cfg);
  const TourPlan b = BuildProceduralTour(world, "", cfg);

  EXPECT_EQ(a.title, b.title);
  EXPECT_EQ(a.seed, b.seed);
  EXPECT_EQ(a.width, b.width);
  EXPECT_EQ(a.height, b.height);
  EXPECT_EQ(a.start.full, b.start.full);
  EXPECT_EQ(a.start.roadTile.x, b.start.roadTile.x);
  EXPECT_EQ(a.start.roadTile.y, b.start.roadTile.y);
  EXPECT_EQ(a.totalSteps, b.totalSteps);
  EXPECT_EQ(a.stops.size(), b.stops.size());

  const std::size_t k = std::min<std::size_t>(a.stops.size(), 8u);
  for (std::size_t i = 0; i < k; ++i) {
    const TourStop& sa = a.stops[i];
    const TourStop& sb = b.stops[i];
    EXPECT_EQ(static_cast<int>(sa.poi.kind), static_cast<int>(sb.poi.kind));
    EXPECT_EQ(sa.poi.name, sb.poi.name);
    EXPECT_EQ(sa.poi.roadTile.x, sb.poi.roadTile.x);
    EXPECT_EQ(sa.poi.roadTile.y, sb.poi.roadTile.y);
    EXPECT_EQ(sa.routeFromPrev.pathCost, sb.routeFromPrev.pathCost);
    EXPECT_EQ(sa.routeFromPrev.maneuvers.size(), sb.routeFromPrev.maneuvers.size());

    const std::size_t mk = std::min<std::size_t>(sa.routeFromPrev.maneuvers.size(), 16u);
    for (std::size_t mi = 0; mi < mk; ++mi) {
      EXPECT_EQ(sa.routeFromPrev.maneuvers[mi].instruction, sb.routeFromPrev.maneuvers[mi].instruction);
    }
  }

  // Poster determinism (route overlay + stop markers).
  TourPosterConfig pcfg;
  pcfg.cartCfg.poster = true;
  pcfg.drawKeyBox = true;
  pcfg.routeLineWidthPx = 2;

  const TourPosterResult pa = RenderTourPoster(world, a, pcfg);
  const TourPosterResult pb = RenderTourPoster(world, b, pcfg);

  EXPECT_EQ(pa.image.width, pb.image.width);
  EXPECT_EQ(pa.image.height, pb.image.height);
  EXPECT_EQ(pa.image.rgba.size(), pb.image.rgba.size());

  const std::uint32_t crca = Crc32(pa.image.rgba.data(), pa.image.rgba.size());
  const std::uint32_t crcb = Crc32(pb.image.rgba.data(), pb.image.rgba.size());
  EXPECT_EQ(crca, crcb);
}
...
int main()
{
  ...
  TestProcGenTerrainPresetAtollCreatesLagoonAndPass();
  TestProcGenTerrainPresetPeninsulaCreatesMainlandEdgeAndTaper();
  ...
  TestStreetNamesAndAddressesDeterministic();
  TestGfxFacilitiesDeterministic();
  TestCartographyPosterDeterministic();
  TestWayfindingRouteDeterministic();
  TestProceduralTourDeterministic();
